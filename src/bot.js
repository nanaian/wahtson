...
module.exports = class Bot extends EventEmitter {
    ...
    constructor(botOptions = {}) {
        ...
        this.client.on('message', this.logRejections(async msg => {
            if (!this.guild) return
            if (msg.guild && msg.guild.id !== this.guild.id) return
            if (msg.author.bot) return
        
            if (await this.config.has('commands')) {
                const { commandAttempted, commandString, commandConfig, args } = await this.parseMessage(msg)
                ...
                if (!commandAttempted) {
                    ...
                    if (commandConfig) {
                        ...
                        await this.executeActionChain(await this.config.get('on_unknown_command'), {
        ...
        this.client.on('guildMemberAdd', this.logRejections(async member => {
            if (!this.guild) return
        ...
        this.client.on('raw', this.logRejections(async packet => {
        ...
        this.client.on('messageReactionAdd', this.logRejections(async (reaction, user) => {
            if (!this.guild) return
            ...
            if (await this.config.has('reactions')) {
                for (const rConfig of await this.config.get('reactions')) {
                    ...
                    if (reaction.emoji.name === wantedEmoji) {
                        this.emit('log', { level: Bot.logLevel.INFO, text: `@${member.displayName} added ${wantedEmoji} reaction` })
        ...
        this.client.on('messageReactionRemove', this.logRejections(async (reaction, user) => {
            if (!this.guild) return
            if (!reaction || reaction.message.guild.id !== this.guild.id) return
            ...
            this.client.once('ready', async () => {
                ...
                if (!this.guild) {
                    if (this.botOptions.promptInvite) {
                        this.emit({ level: Bot.logLevel.WARN, text: 'Bot is not present in configured server!' })
                        this.emit({ level: Bot.logLevel.WARN, text: 'Please invite it using your browser.' })
                ...
                this.emit('log', { level: Bot.logLevel.INFO, text: 'Server found. Listening for commands...' })
                resolve()
            })
        })
    ...
    async executeActionChain(actions, source) {
        ...
        for (let idx = 0; idx < actions.length; idx++) {
            ...
            if (!fn) {
                this.emit('log', { level: Bot.logLevel.ERROR, text: `Unknown action type "${action.type}"` })
                continue
            }
    ...
    async handlePossiblePin(reaction) {
        ...
        if (
            ...
            if (!isPinned) {
                await db.run(
                    sql`INSERT INTO pins VALUES (${reaction.message.id})`
                )
    ...
    logRejections(promiseFn) {
        return (...args) => promiseFn(...args).catch(err => {
            this.emit('log', {
                level: Bot.logLevel.ERROR,
                text: err.toString(),
            })
